...
test_that("aggregations", {
  ...
  expect_equal(
    agg_dynamic_balance$att.egt[adb_idx] -
      agg_dynamic_balance$att.egt[adb_idx - 1],
    1,
    tol = .5
  )
})
...
test_that("significance level and uniform confidence bands", {
  ...
  expect_lte(
    res05$att[1] + res05$c * res05$se[1],
...
test_that("custom estimation method", {
  ...
  res <- att_gt(
    ...
    idname = "id",
    gname = "G",
    est_method = DRDID::drdid_imp_panel,
    panel = TRUE
  )
  expect_equal(res$att[1], 1, tol = .5)
})
...
test_that("clustered standard errors", {
  ...
  expect_error(
    ...
    "handle time-varying cluster variables"
  )
...
test_that("faster model enabled for repeated cross sectional data", {
  data_rcs <- as.data.table(did::build_sim_dataset(
    reset.sim(time.periods = 4, n = 1000),
    panel = FALSE
  ))
...
test_that("faster_mode time indexing matches baseline with repeated cross-sections", {
  ...
  rep_by_t <- sapply(1:n_periods, function(j) {
    floor(2 + 3 * (1.2)^(n_periods - j) * runif(1))
  })
...
test_that("faster_mode time indexing with non-consecutive time periods", {
  ...
  expect_true(
    all(res_slow$t %in% time_periods),
    info = "Time values should be actual calendar years"
  )
  expect_true(
    all(res_fast$t %in% time_periods),
  ...
  expect_true(
    min(res_slow$t) > min(time_periods),
