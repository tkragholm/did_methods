...
load_jel_data <- function(path, filter_2xt = TRUE) {
  ...
  if (filter_2xt) {
    ...
    mydata <- mydata[
      mydata$yaca == 2014 | is.na(mydata$yaca) | mydata$yaca > 2019,
    ]
  }

  # Compute covariates
  mydata$perc_white <- mydata$population_20_64_white /
    mydata$population_20_64 *
    100
  mydata$perc_hispanic <- mydata$population_20_64_hispanic /
    mydata$population_20_64 *
    100
  mydata$perc_female <- mydata$population_20_64_female /
    mydata$population_20_64 *
    100
  ...
  county_year_counts <- tapply(
    mydata$year %in% c(2013, 2014),
    mydata$county_code,
    sum
  )
...
test_that("JEL Table 7: 2x2 CS-DiD point estimates match", {
  ...
  short_data$treat_year <- ifelse(
    !is.na(short_data$yaca) & short_data$yaca == 2014,
    2014,
    0
  )
  ...
  wt_2013 <- short_data[
    short_data$year == 2013,
    c("county_code", "population_20_64")
  ]
  names(wt_2013)[2] <- "set_wt"
  short_data <- merge(short_data, wt_2013, by = "county_code")

  covs_formula <- ~ perc_female +
    perc_white +
    perc_hispanic +
    unemp_rate +
    poverty_rate +
  ...
  for (method in c("reg", "ipw", "dr")) {
    for (wt_info in list(
      list(name = NULL, label = "unweighted"),
...
test_that("JEL 2xT: event study ATT(g,t) point estimates match", {
  ...
  mydata <- load_jel_data(jel_data_path, filter_2xt = TRUE)
  mydata$treat_year <- ifelse(
    !is.na(mydata$yaca) & mydata$yaca == 2014,
    2014,
    0
  )
...
test_that("JEL 2xT: event study with covariates matches across methods", {
  ...
  for (method in c("reg", "ipw", "dr")) {
    ...
    expect_equal(
      es$att.egt[base_idx],
      0,
      label = paste("2xT covs", method, "e=-1")
    )

    # All estimates should be finite
    expect_true(
      all(is.finite(es$att.egt[!is.na(es$att.egt)])),
...
test_that("JEL GxT: staggered event study without covariates matches", {
  ...
  mydata$treat_year <- ifelse(
    !is.na(mydata$yaca) & mydata$yaca <= 2019,
    mydata$yaca,
    0
  )
  mydata$county_code <- as.numeric(mydata$county_code)
...
test_that("JEL: faster_mode matches regular mode", {
  ...
  expect_equal(
    agg_slow$overall.att,
    agg_fast$overall.att,
    tolerance = 1e-10,
