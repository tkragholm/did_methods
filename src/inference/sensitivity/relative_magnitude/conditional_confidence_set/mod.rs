...
impl<'a> RelativeMagnitudeThetaEvaluator<'a> {
    ...
    fn shift_to(&mut self, theta: f64) {
        for (out, (y, a)) in self
            .shifted_y_arp
            .iter_mut()
    ...
    /// The ARP statistic at `theta`, and nothing else.
    ///
    /// `eta*` is the optimal value of an LP whose right-hand side is AFFINE in
    /// `theta`, and partial minimisation preserves convexity, so `eta*` is a
    /// CONVEX function of `theta`. That is what [`Self::argmin_eta`] rests on.
    fn eta_at(&mut self, theta: f64) -> Result<f64, String> {
        self.shift_to(theta);
        self.workspace.solve_eta_only(&self.shifted_y_arp)
    }

    /// The grid index minimising the ARP statistic.
    ///
    /// For a convex sequence the forward differences are non-decreasing, so the
    /// first index whose forward difference is non-negative is a minimiser, and
    /// binary search finds it in `2*log2(n)` probes rather than `n`. A plateau
    /// is fine: the leftmost minimiser is returned, and any minimiser will do.
    fn argmin_eta(&mut self, grid: &[f64]) -> Result<(usize, f64), String> {
        let (mut lo, mut hi) = (0usize, grid.len() - 1);
        while lo < hi {
            let mid = usize::midpoint(lo, hi);
            let here = self.eta_at(grid[mid])?;
            let next = self.eta_at(grid[mid + 1])?;
            if next >= here {
                hi = mid;
            } else {
                lo = mid + 1;
...
/// Where to start this branch's acceptance search.
///
/// Only a starting point: the search below is exact wherever it starts, and a
/// bad anchor costs evaluations rather than accuracy.
///
/// `identified` is this branch's own identified set, and it is `None` when the
/// branch's identified-set LP was infeasible. **That is not a reason to skip
/// the branch.** The identified-set LP fixes `delta_pre = betahat_pre`, and
/// under `DeltaRM` the branch index names which pre-period attains the largest
/// violation, so at any one realised `betahat_pre` exactly one branch can be
/// feasible: on Study I's shape, one of eight. The CONFIDENCE set is not
/// conditioned on the realised pre-trend — the pre-period coefficients are
/// random, every branch's moment inequalities are tested against their sampling
/// covariance, and the confidence set is the union over all of them. Dropping
/// the branches that the realised pre-trend rules out is conditioning on that
/// pre-trend, which is the pre-testing distortion the whole method exists to
/// avoid, and it returns intervals that are too narrow.
///
/// This path did skip them until 4 September 2026. Against
/// `tests/honest_did_joint_path_ref.json`, generated by R's
/// `HonestDiD::createSensitivityResults_relativeMagnitudes`, skipping was
/// 0.005117 from R at worst and searching every branch is 0.002909;
/// `honest_joint_path_region_bonferroni_matches_r_fixture` did not catch it
/// because its tolerance was `1e-2`, wider than the 0.0046 the skip moved that
/// bound. On Study I's own shape it narrowed 160 of 180 intervals, by up to
/// 17.5% of their width.
fn branch_anchor_idx(
    ...
    let Some(identified) = identified else {
        // Nothing better to go on than the point estimate.
        return nearest_grid_index(grid, original_estimate);
    };
...
pub(in crate::inference::sensitivity) fn compute_relative_magnitude_confidence_set_for_basis_period_with_prepared_input(
    ...
    {
        // Every branch is searched, including one whose identified set came
        // back infeasible. See `branch_anchor_idx` for why that is not a
        // licence to skip it.
        let anchor_idx = branch_anchor_idx(&grid, original.estimate, branch_identified.as_ref());
...
fn compute_relative_magnitude_branch_accepted_range_for_branch(
    ...
    let anchor_idx = better_anchor_idx(grid, anchor_idx, &mut evaluator)?;
    let Some(anchor_idx) = anchor_idx else {
        return Ok((None, AdaptiveGridDiagnostics::default()));
    };
    compute_relative_magnitude_branch_accepted_range_adaptive(grid, anchor_idx, &mut evaluator)
}

/// Where the acceptance search should start, and whether it is worth starting.
///
/// `Ok(None)` means the branch accepts nothing and the caller may move on.
///
/// The supplied anchor is tried first and kept when it is accepted, which is
/// the common case and costs one evaluation. It is the point estimate, or this
/// branch's own identified-set midpoint, and neither knows anything about the
/// branch's acceptance region -- so when it is rejected the adaptive search
/// walks outward from it one index at a time looking for a point that is not,
/// and then, having found one on a side, scans the whole other side to rule out
/// a second component. Both are linear. Measured on Study I's shape that is
/// where the cost of the whole inversion sits: 20% of branch searches ended in
/// the 1,000-point fallback and they accounted for 91% of every evaluation
/// made.
///
/// So when the anchor is rejected, spend `2*log2(n)` cheap statistic-only
/// solves finding the minimiser of `eta*` instead. It is the single best
/// candidate available -- the point that is hardest to reject -- and if the
/// least-favorable first stage rejects even there, convexity says it rejects
/// everywhere and the branch is empty without another solve. Same intervals,
/// 311,490 evaluations down to 171,446 on the regime that dominates production.
fn better_anchor_idx(
    grid: &[f64],
    anchor_idx: usize,
    evaluator: &mut RelativeMagnitudeThetaEvaluator<'_>,
) -> Result<Option<usize>, String> {
    let anchor_idx = anchor_idx.min(grid.len() - 1);
    if evaluator.accepts(grid[anchor_idx])? {
        return Ok(Some(anchor_idx));
    }
    let (best_idx, best_eta) = evaluator.argmin_eta(grid)?;
    if evaluator.lf_cv.is_finite() && best_eta > evaluator.lf_cv {
        return Ok(None);
    }
    Ok(Some(best_idx))
}
